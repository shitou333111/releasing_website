...
async function run() {
  ...
  if (!SKIP_MFA) {
    try {
      ...
      const mfaScript = path.join(docsRoot, '.vitepress', 'gen-tts-align-mfa.py');
      ...
      const postproc = `node "${path.join(docsRoot, '.vitepress', 'postprocess-unaligned-cues.cjs')}" "${articleOutputDir}"`;
      console.log('Running postprocess:', postproc);
      execSync(postproc, { stdio: 'inherit' });
      const merge = `node "${path.join(docsRoot, '.vitepress', 'merge-segments-and-cues.cjs')}" "${articleOutputDir}"`;
